  ...
  return (
    <AppShell activeHref="/ai-insights">
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-2">
        <Block title="输入数据摘要" items={["美团推广数据：已支持计算，参与计算", "e看牙承接/成交数据：已支持计算，参与回流判断", "项目价格/成本表：已支持计算，参与毛利和目标CPA判断", "抖音、腾讯广点通：入口已开放，暂不参与计算", "竞品情报：入口已开放，仅作市场参考"]} />
        <Block title="规则判断结果" items={["平台线索回流率偏低，建议置信度需要降低", "成交金额完整率不足，不建议重大价格或预算调整", "高客单项目观察周期不足时，只建议继续观察"]} />
        <Block title="AI生成建议" items={["解释为什么种植可小幅加预算，但不能大幅放量", "解释为什么洁牙优先优化价格表达，而不是直接降价", "整理明日动作清单，提醒人工确认"]} />
