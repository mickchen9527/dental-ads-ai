...
export async function POST(request: Request) {
  ...
  if (!supportedDataTypes.has(fileResult.data.data_type ?? "")) {
    return NextResponse.json({ message: "当前 V1.7.5 只支持解析腾讯广告组/创意数据。" }, { status: 400 });
  }
  ...
  if (downloadResult.error || !downloadResult.data) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-gdt-creatives] Storage download failed", {
  ...
  try {
    ...
    if (!hasGdtCreativeKeyFields(parsedData.headers)) {
      await markUploadFailed(supabase, id);
      return NextResponse.json(
        { message: "这个文件不像腾讯广告组/创意数据，缺少广告组、创意、消耗、曝光、点击或转化相关字段。请确认你上传的是腾讯广告组/创意报表，不是计划汇总表或线索表。" },
        { status: 400 },
      );
    }
    ...
    if (parsedData.rows.length === 0) {
      await markUploadFailed(supabase, id);
      return NextResponse.json({ message: "文件里没有可解析的数据行，请检查是否上传了腾讯广告组/创意报表。" }, { status: 400 });
    }

    const mappedRows = parsedData.rows.map((row) => mapGdtCreativeRow(id, row));
    const deleteResult = await supabase.from("gdt_creative_rows").delete().eq("uploaded_file_id", id);

    if (deleteResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-gdt-creatives] old rows delete failed", {
      ...
      return NextResponse.json({ message: "重新解析前清理旧数据失败，请检查 gdt_creative_rows 表权限。" }, { status: 500 });
    }

    const insertResult = await supabase.from("gdt_creative_rows").insert(mappedRows);

    if (insertResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-gdt-creatives] rows insert failed", {
      ...
      return NextResponse.json({ message: "腾讯广告组/创意明细写入失败，请检查 gdt_creative_rows 表字段。" }, { status: 500 });
    }
    ...
    if (updateResult.error) {
      console.error("[api/uploads/parse-gdt-creatives] uploaded_files parsed update failed", {
    ...
    return NextResponse.json({
      ...
      message: `腾讯广告组/创意数据解析成功，共写入 ${mappedRows.length} 行。`,
    });
  } catch (error) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-gdt-creatives] parse failed", {
      message: error instanceof Error ? error.message : "unknown parse error",
      originalFileName: fileResult.data.original_file_name,
    });

    return NextResponse.json({ message: "腾讯广告组/创意数据解析失败，请确认文件格式和表头是否正确。" }, { status: 500 });
  }
}
...
function mapGdtCreativeRow(uploadedFileId: string, row: RawSheetRow) {
  return {
...
function hasGdtCreativeKeyFields(headers: string[]) {
  const mappedFields = new Set(headers.map((header) => headerMap[normalizeHeader(header)]).filter(Boolean));
  const groups = [
    ["ad_group_name", "creative_name", "creative_id", "material_name", "material_id"],
    ["spend"],
    ["impressions"],
    ["clicks"],
