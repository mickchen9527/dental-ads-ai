...
const supportedDataTypes = new Set(["抖音表单/私信线索数据", "抖音表单 / 私信线索数据", "douyin-leads"]);
...
const headerAliases: Array<[string, string]> = [
  ...
  ["感兴趣项目", "intention_project"],
  ["咨询内容项目", "intention_project"],
  ["私信内容", "message_content"],
  ["咨询内容", "message_content"],
  ["留言内容", "message_content"],
  ["表单内容", "message_content"],
  ["用户留言", "message_content"],
  ["需求描述", "message_content"],
  ["沟通内容", "message_content"],
  ["跟进状态", "follow_status"],
  ["处理状态", "follow_status"],
...
const keyFieldCategories = [
  ...
  { label: "咨询内容类", fields: ["message_content", "remark"] },
];
...
export async function POST(request: Request) {
  ...
  if (!supportedDataTypes.has(fileResult.data.data_type ?? "")) {
    return NextResponse.json({ message: "当前 V1.7.3 只支持解析抖音表单/私信线索数据。" }, { status: 400 });
  }
  ...
  if (downloadResult.error || !downloadResult.data) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-douyin-leads] Storage download failed", {
  ...
  try {
    ...
    const headerDiagnosis = diagnoseDouyinLeadHeaders(parsedData.headers);

    if (!hasDouyinLeadKeyFields(headerDiagnosis)) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-douyin-leads] header validation failed", {
      ...
      return NextResponse.json(
        { message: buildHeaderFailureMessage(parsedData.headers, headerDiagnosis) },
        { status: 400 },
      );
    }
    ...
    if (parsedData.rows.length === 0) {
      await markUploadFailed(supabase, id);
      return NextResponse.json({ message: "文件里没有可解析的数据行，请检查是否上传了抖音线索明细表。" }, { status: 400 });
    }

    const mappedRows = parsedData.rows.map((row) => mapDouyinLeadRow(id, row));
    const deleteResult = await supabase.from("douyin_lead_rows").delete().eq("uploaded_file_id", id);

    if (deleteResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-douyin-leads] old rows delete failed", {
      ...
      return NextResponse.json({ message: "重新解析前清理旧数据失败，请检查 douyin_lead_rows 表权限。" }, { status: 500 });
    }

    const insertResult = await supabase.from("douyin_lead_rows").insert(mappedRows);

    if (insertResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-douyin-leads] rows insert failed", {
      ...
      return NextResponse.json({ message: "抖音表单/私信线索明细写入失败，请检查 douyin_lead_rows 表字段。" }, { status: 500 });
    }
    ...
    if (updateResult.error) {
      console.error("[api/uploads/parse-douyin-leads] uploaded_files parsed update failed", {
    ...
    return NextResponse.json({
      ...
      message: `抖音表单/私信线索数据解析成功，共写入 ${mappedRows.length} 行。`,
    });
  } catch (error) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-douyin-leads] parse failed", {
      message: error instanceof Error ? error.message : "unknown parse error",
      originalFileName: fileResult.data.original_file_name,
    });

    return NextResponse.json({ message: "抖音表单/私信线索数据解析失败，请确认文件格式和表头是否正确。" }, { status: 500 });
  }
}
...
function findHeaderRowIndex(headerRows: CellValue[][]) {
  ...
  const likelyHeaderRow = nonEmptyRows.find((item) => hasDouyinLeadKeyFields(diagnoseDouyinLeadHeaders(item.headers)));
...
function mapDouyinLeadRow(uploadedFileId: string, row: RawSheetRow) {
  ...
  const messageContent = parseTextCell(getCell(row, "message_content")) ?? parseTextCell(getCell(row, "remark"));
...
function hasDouyinLeadKeyFields(diagnosis: ReturnType<typeof diagnoseDouyinLeadHeaders>) {
  return diagnosis.matchedCategories.length >= 2;
}

function diagnoseDouyinLeadHeaders(headers: string[]) {
...
function buildHeaderFailureMessage(headers: string[], diagnosis: ReturnType<typeof diagnoseDouyinLeadHeaders>) {
  ...
  return `解析失败：当前读取到的表头是：${headerText}。已识别：${matchedText}。缺失：${missingText}。请确认你上传的是抖音线索明细表，不是计划汇总表或素材表。`;
}
