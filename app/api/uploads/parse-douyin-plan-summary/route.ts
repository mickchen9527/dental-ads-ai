...
const supportedDataTypes = new Set(["抖音计划汇总数据", "抖音广告计划汇总数据", "douyin-plan-summary", "douyin-ad-plan-summary"]);
...
export async function POST(request: Request) {
  ...
  if (!supportedDataTypes.has(fileResult.data.data_type ?? "")) {
    return NextResponse.json({ message: "当前 V1.7.1 只支持解析抖音计划汇总数据。" }, { status: 400 });
  }
  ...
  if (downloadResult.error || !downloadResult.data) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-douyin-plan-summary] Storage download failed", {
  ...
  try {
    ...
    if (!hasDouyinPlanKeyFields(parsedData.headers)) {
      await markUploadFailed(supabase, id);
      return NextResponse.json(
        { message: "这个文件不像抖音计划汇总数据，缺少计划名称、消耗、展示、点击或转化相关字段。请确认你上传的是抖音广告计划汇总报表，不是素材表或线索表。" },
        { status: 400 },
      );
    }
    ...
    if (parsedData.rows.length === 0) {
      await markUploadFailed(supabase, id);
      return NextResponse.json({ message: "文件里没有可解析的数据行，请检查是否上传了抖音广告计划汇总报表。" }, { status: 400 });
    }

    const mappedRows = parsedData.rows.map((row) => mapDouyinPlanSummaryRow(id, row));
    const deleteResult = await supabase.from("douyin_plan_summary_rows").delete().eq("uploaded_file_id", id);

    if (deleteResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-douyin-plan-summary] old rows delete failed", {
      ...
      return NextResponse.json({ message: "重新解析前清理旧数据失败，请检查 douyin_plan_summary_rows 表权限。" }, { status: 500 });
    }

    const insertResult = await supabase.from("douyin_plan_summary_rows").insert(mappedRows);

    if (insertResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-douyin-plan-summary] rows insert failed", {
      ...
      return NextResponse.json({ message: "抖音计划汇总明细写入失败，请检查 douyin_plan_summary_rows 表字段。" }, { status: 500 });
    }
    ...
    if (updateResult.error) {
      console.error("[api/uploads/parse-douyin-plan-summary] uploaded_files parsed update failed", {
    ...
    return NextResponse.json({
      ...
      message: `抖音计划汇总数据解析成功，共写入 ${mappedRows.length} 行。`,
    });
  } catch (error) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-douyin-plan-summary] parse failed", {
      message: error instanceof Error ? error.message : "unknown parse error",
      originalFileName: fileResult.data.original_file_name,
    });

    return NextResponse.json({ message: "抖音计划汇总数据解析失败，请确认文件格式和表头是否正确。" }, { status: 500 });
  }
}
...
function mapDouyinPlanSummaryRow(uploadedFileId: string, row: RawSheetRow) {
  return {
...
function hasDouyinPlanKeyFields(headers: string[]) {
  ...
  const groups = [
    ...
    ["impressions"],
    ["clicks"],
    ["conversions", "form_count", "private_message_count", "phone_count"],
  ];
