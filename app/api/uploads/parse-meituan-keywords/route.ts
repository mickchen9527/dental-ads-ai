...
const supportedDataTypes = new Set(["美团关键词数据", "meituan-keywords"]);
...
export async function POST(request: Request) {
  ...
  if (!supportedDataTypes.has(fileResult.data.data_type ?? "")) {
    return NextResponse.json({ message: "当前 V1.6.3.2 只支持解析美团关键词数据。" }, { status: 400 });
  }
  ...
  if (downloadResult.error || !downloadResult.data) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-meituan-keywords] Storage download failed", {
  ...
  try {
    ...
    if (!hasKeywordHeader(parsedData.headers)) {
      await markUploadFailed(supabase, id);
      return NextResponse.json(
        { message: "这个文件不像美团关键词数据，缺少‘关键词’字段。请确认你上传的是关键词报表，不是美团推广汇总数据。" },
        { status: 400 },
      );
    }
    ...
    if (parsedData.rows.length === 0) {
      await markUploadFailed(supabase, id);
      return NextResponse.json({ message: "文件里没有可解析的数据行，请检查是否上传了美团关键词报表。" }, { status: 400 });
    }

    const mappedRows = parsedData.rows.map((row) => mapMeituanKeywordRow(id, row));
    const deleteResult = await supabase.from("meituan_keyword_rows").delete().eq("uploaded_file_id", id);

    if (deleteResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-meituan-keywords] old rows delete failed", {
      ...
      return NextResponse.json({ message: "重新解析前清理旧数据失败，请检查 meituan_keyword_rows 表权限。" }, { status: 500 });
    }

    const insertResult = await supabase.from("meituan_keyword_rows").insert(mappedRows);

    if (insertResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-meituan-keywords] rows insert failed", {
      ...
      return NextResponse.json({ message: "美团关键词明细写入失败，请检查 meituan_keyword_rows 表字段。" }, { status: 500 });
    }
    ...
    if (updateResult.error) {
      console.error("[api/uploads/parse-meituan-keywords] uploaded_files parsed update failed", {
    ...
    return NextResponse.json({
      ...
      message: `美团关键词数据解析成功，共写入 ${mappedRows.length} 行。`,
    });
  } catch (error) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-meituan-keywords] parse failed", {
      message: error instanceof Error ? error.message : "unknown parse error",
      originalFileName: fileResult.data.original_file_name,
    });

    return NextResponse.json({ message: "美团关键词数据解析失败，请确认文件格式和表头是否正确。" }, { status: 500 });
  }
}
...
function mapMeituanKeywordRow(uploadedFileId: string, row: RawSheetRow) {
  return {
    ...
    bid_price: parseNumberCell(getCell(row, "bid_price")),
    match_type: parseTextCell(getCell(row, "match_type")),
    keyword_status: parseTextCell(getCell(row, "keyword_status")),
    raw_row: row,
  };
}

function hasKeywordHeader(headers: string[]) {
  return headers.some((header) => headerMap[normalizeHeader(header)] === "keyword");
}
