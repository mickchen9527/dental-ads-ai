...
const supportedDataTypes = new Set(["抖音素材/创意数据", "抖音素材 / 创意数据", "douyin-creatives"]);
...
const headerAliases: Array<[string, string]> = [
  ...
  ["播放量", "play_count"],
  ["播放次数", "play_count"],
  ["有效播放", "valid_play_count"],
  ["有效播放量", "valid_play_count"],
  ["完播量", "complete_play_count"],
  ["完播次数", "complete_play_count"],
  ["完播率", "complete_play_rate"],
  ["转化", "conversions"],
  ["转化数", "conversions"],
  ["转化量", "conversions"],
  ...
  ["表单提交", "form_count"],
  ["留资数", "form_count"],
  ["私信", "private_message_count"],
  ["私信数", "private_message_count"],
  ["私信咨询", "private_message_count"],
  ["电话", "phone_count"],
  ["电话数", "phone_count"],
  ["电话咨询", "phone_count"],
];
...
export async function POST(request: Request) {
  ...
  if (!supportedDataTypes.has(fileResult.data.data_type ?? "")) {
    return NextResponse.json({ message: "当前 V1.7.2 只支持解析抖音素材/创意数据。" }, { status: 400 });
  }
  ...
  if (downloadResult.error || !downloadResult.data) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-douyin-creatives] Storage download failed", {
  ...
  try {
    ...
    if (!hasDouyinCreativeKeyFields(parsedData.headers)) {
      await markUploadFailed(supabase, id);
      return NextResponse.json(
        { message: "这个文件不像抖音素材/创意数据，缺少创意名称、素材名称、视频名称、消耗、展示、点击或播放/转化相关字段。请确认你上传的是抖音素材/创意报表，不是计划汇总表或线索表。" },
        { status: 400 },
      );
    }
    ...
    if (parsedData.rows.length === 0) {
      await markUploadFailed(supabase, id);
      return NextResponse.json({ message: "文件里没有可解析的数据行，请检查是否上传了抖音素材/创意报表。" }, { status: 400 });
    }

    const mappedRows = parsedData.rows.map((row) => mapDouyinCreativeRow(id, row));
    const deleteResult = await supabase.from("douyin_creative_rows").delete().eq("uploaded_file_id", id);

    if (deleteResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-douyin-creatives] old rows delete failed", {
      ...
      return NextResponse.json({ message: "重新解析前清理旧数据失败，请检查 douyin_creative_rows 表权限。" }, { status: 500 });
    }

    const insertResult = await supabase.from("douyin_creative_rows").insert(mappedRows);

    if (insertResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-douyin-creatives] rows insert failed", {
      ...
      return NextResponse.json({ message: "抖音素材/创意明细写入失败，请检查 douyin_creative_rows 表字段。" }, { status: 500 });
    }
    ...
    if (updateResult.error) {
      console.error("[api/uploads/parse-douyin-creatives] uploaded_files parsed update failed", {
    ...
    return NextResponse.json({
      ...
      message: `抖音素材/创意数据解析成功，共写入 ${mappedRows.length} 行。`,
    });
  } catch (error) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-douyin-creatives] parse failed", {
      message: error instanceof Error ? error.message : "unknown parse error",
      originalFileName: fileResult.data.original_file_name,
    });

    return NextResponse.json({ message: "抖音素材/创意数据解析失败，请确认文件格式和表头是否正确。" }, { status: 500 });
  }
}
...
function mapDouyinCreativeRow(uploadedFileId: string, row: RawSheetRow) {
  return {
    ...
    material_name: parseTextCell(getCell(row, "material_name")),
    video_name: parseTextCell(getCell(row, "video_name")),
    creative_id: parseTextCell(getCell(row, "creative_id")),
    material_id: parseTextCell(getCell(row, "material_id")),
    ...
    play_count: parseIntegerCell(getCell(row, "play_count")),
    valid_play_count: parseIntegerCell(getCell(row, "valid_play_count")),
    complete_play_count: parseIntegerCell(getCell(row, "complete_play_count")),
    complete_play_rate: parsePercentCell(getCell(row, "complete_play_rate")),
    ...
    private_message_count: parseIntegerCell(getCell(row, "private_message_count")),
    phone_count: parseIntegerCell(getCell(row, "phone_count")),
    raw_row: row,
  };
}

function hasDouyinCreativeKeyFields(headers: string[]) {
  const mappedFields = new Set(headers.map((header) => headerMap[normalizeHeader(header)]).filter(Boolean));
  const groups = [
    ["creative_name", "material_name", "video_name"],
    ["spend"],
    ["impressions"],
    ["clicks"],
    ["play_count", "complete_play_rate", "conversions", "form_count", "private_message_count", "phone_count"],
  ];
