...
const supportedDataTypes = new Set(["高德线索数据", "高德留资数据", "高德咨询线索数据", "高德客户线索数据", "amap-leads"]);
...
export async function POST(request: Request) {
  ...
  if (!supportedDataTypes.has(fileResult.data.data_type ?? "")) {
    return NextResponse.json({ message: "当前 V1.7.9 只支持解析高德线索数据。" }, { status: 400 });
  }
  ...
  if (downloadResult.error || !downloadResult.data) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-amap-leads] Storage download failed", {
  ...
  try {
    ...
    const headerDiagnosis = diagnoseAmapLeadHeaders(parsedData.headers);

    if (!hasAmapLeadKeyFields(headerDiagnosis)) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-amap-leads] header validation failed", {
    ...
    if (parsedData.rows.length === 0) {
      await markUploadFailed(supabase, id);
      return NextResponse.json({ message: "文件里没有可解析的数据行，请检查是否上传了高德线索明细表。" }, { status: 400 });
    }

    const mappedRows = parsedData.rows.map((row) => mapAmapLeadRow(id, row));
    const deleteResult = await supabase.from("amap_lead_rows").delete().eq("uploaded_file_id", id);

    if (deleteResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-amap-leads] old rows delete failed", {
      ...
      return NextResponse.json({ message: `清理旧高德线索数据失败：${deleteResult.error.message}` }, { status: 500 });
    }

    const insertResult = await supabase.from("amap_lead_rows").insert(mappedRows);

    if (insertResult.error) {
      await markUploadFailed(supabase, id);
      console.error("[api/uploads/parse-amap-leads] rows insert failed", {
      ...
      return NextResponse.json({ message: "高德线索明细写入失败，请检查 amap_lead_rows 表字段。" }, { status: 500 });
    }
    ...
    if (updateResult.error) {
      console.error("[api/uploads/parse-amap-leads] uploaded_files parsed update failed", {
    ...
    return NextResponse.json({
      ...
      message: `高德线索数据解析成功，共写入 ${mappedRows.length} 行。`,
    });
  } catch (error) {
    await markUploadFailed(supabase, id);
    console.error("[api/uploads/parse-amap-leads] parse failed", {
      message: error instanceof Error ? error.message : "unknown parse error",
      originalFileName: fileResult.data.original_file_name,
    });

    return NextResponse.json({ message: "高德线索数据解析失败，请确认文件格式和表头是否正确。" }, { status: 500 });
  }
}
...
function findHeaderRowIndex(headerRows: CellValue[][]) {
  ...
  const likelyHeaderRow = nonEmptyRows.find((item) => hasAmapLeadKeyFields(diagnoseAmapLeadHeaders(item.headers)));
...
function mapAmapLeadRow(uploadedFileId: string, row: RawSheetRow) {
...
function hasAmapLeadKeyFields(diagnosis: ReturnType<typeof diagnoseAmapLeadHeaders>) {
  return diagnosis.matchedCategories.length >= 2;
}

function diagnoseAmapLeadHeaders(headers: string[]) {
...
function buildHeaderFailureMessage(headers: string[], diagnosis: ReturnType<typeof diagnoseAmapLeadHeaders>) {
  ...
  return `解析失败：当前读取到的表头是：${headerText}。已识别：${matchedText}。缺失：${missingText}。请确认你上传的是高德线索明细表，不是推广汇总表或行为明细表。`;
}
